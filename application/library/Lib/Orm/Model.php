...
class Model {
    ...
    private function connect($type = 'WRITE') {
        ...
        try{
            ...
            abort("{$e->getCode()} => {$e->getMessage()}", StatusSqlError);
        }
    }
    ...
    final public function all($boole = false) {
        $boole ? $this->options['limit'] = null : null;
        return $this;
    }
    ...
    public function fetchSql($boole = false) {
        $this->options['fetchsql'] = $boole;
        return $this;
    }
    ...
    final public function select(){
        ...
        if (isset($this->options['total']) && $this->options['total']) {
            $this->sql = " select count(*) as total from ({$this->sql}) s";
            $this->total = true;
        }
        // 连接DB
        if (isset($this->options['fetchsql']) && $this->options['fetchsql']) {
            return $this->sql;
        }
        //echo $this->sql.BR.BR;
        $this->connect('READ');
    ...
    public function alice($alice_name = '') {
        $alice_name ? ($this->options['alice'] = $alice_name) : null;
        return $this;
    }

    public function join($table='', $alice='', $condition='', $join='LEFT') {
        $tableName = $this->getTablePrefix().$table;
        $join = " {$join} JOIN `{$tableName}` `{$alice}` ON  {$condition} ";
        is_null($this->options['join']) ? ($this->options['join'] = array()) : null;
        array_push($this->options['join'], $join);
        return $this;
    }

    public function group($group='') {
        $this->options['group'] = $group ? $group : null;
        return $this;
    }
    ...
    final protected function generateSQL(){
        ...
        $sql = 'SELECT '. $field .' FROM `'. $this->getTableName(). '` ' .(isset($this->options['alice']) ? " `{$this->options['alice']}` " : null);
     ...
     */
    final public function total($bool = true) {
        if ($bool) {
            $this->options['total'] = true;
            unset($this->options['order']);
            unset($this->options['limit']);
            $this->options['field'] = explode(',', $this->options['field'])[0];
        }
        return $this;
    }
    ...
    /*public function getTableName()
    {
        ...
        if (!$this->table) {
            ...
            $this->table = toUnderScore($className);
        }
    ...
    {
        $this->table = $this->getTablePrefix().$table_name;
        return $table_name ? $table_name : 0;
    }
